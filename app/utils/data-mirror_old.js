 ...
 */
function getMirrorPath(objComposite, path) {
    var rtnMPath = [];

    function findRecursor(objComposite, currentPath){
        ...
        if (arrPath.length !== 0) {
            if (subLevelObj.key === curPath) {
                findRecursor(subLevelObj.value, arrPath.join('.'));
            } else {
                findRecursor(subLevelObj, arrPath.join('.'));
            }
        }
    }

    findRecursor(objComposite, path);

    return rtnMPath.join('.');
}

function retObjPath(path){
...
function getByMirrorPath(objComposite, mirrorPath) {
...
function objCompGet(objComposite, path) {
    var rtnObj = null,
        pattern = /\[([0-9])\]/g,
        tempVal,
        arrIdx,
    ...
    if (arrPath.length === 0){
        ...
        if (tempObj && !tempObj._isComp && !_.isArray(tempObj)) {
            if (tempObj.key === curPath) { // KVObject
                rtnObj = objCompGet(tempObj.value, arrPath.join("."));
            } else {
                console.log("new edge case");
            }
        } else {
            rtnObj = objCompGet(tempObj, arrPath.join('.'));
        }
...
module.exports = {
    ...
    cloneByPath: objCompGet,
    getMirrorRoute: getMirrorPath,
    getByRoute : getByMirrorPath,
    routeToPath: retObjPath,
